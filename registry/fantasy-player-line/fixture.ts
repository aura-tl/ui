...
// Game: nfl:game:401873272 — DET @ CIN, live preseason, 2026-08-13. Player: Jack Endries (CIN).
// Never fabricate fixture data — recapture from a real board instead.
import { toAuraFantasyPlayerLineModel, type AuraFantasyPlayerLineState } from '@/lib/aura/fantasy-player-line-types';
...
const model = toAuraFantasyPlayerLineModel(boxScore, { playerId: "jack endries" });

export const auraFantasyPlayerLineFixture: AuraFantasyPlayerLineState = model
  ? { status: 'ready', error: null, model }
  : { status: 'unavailable', error: 'Player line not present in the captured box score.', model: null };
